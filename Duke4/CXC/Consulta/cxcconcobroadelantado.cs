...
{
    public partial class cxcconcobroadelantado : Duke4.MainFormConsulta
    {
        public cxcconcobroadelantado()
        {
            InitializeComponent();
            string cmd = string.Format(queryconsulta + " cxcdbfcobroadelantado.nulo=0 and cast(fecha as date) = cast(getdate() as date) order by secuencia");
            DataSet ds = FuncionesSQL.Fun_Sql_Ejecutar(cmd);
            gridBase1.DataSource = ds.Tables[0];
            Fun_Calcular_Totales();
        }
        
        string queryconsulta = "SELECT     secuencia, fecha,cxcdbfcobroadelantado.referencia,nombre,balance,monto,idcobro,cxcdbfcobroadelantado.nulo,'' as entrada "
        + "FROM dbo.cxcdbfcobroadelantado "
        + "inner JOIN dbo.cxcdbfcliente on cxcdbfcliente.idcliente= cxcdbfcobroadelantado.idcliente where ";
        ...
        protected override void Filtrar()
        {
            base.Filtrar();
            txtMayuscula1.Text = "";
            string cmd = string.Format(queryconsulta + " fecha >= Case When '" + Funciones_Duke4.Funciones.Fun_Convertir_Fecha_FormatoyyyMMdd(dtpBase1) + "' <> ' ' Then '" + Funciones_Duke4.Funciones.Fun_Convertir_Fecha_FormatoyyyMMdd(dtpBase1) + "' Else fecha End  "
            + " and fecha <= Case When '" + Funciones_Duke4.Funciones.Fun_Convertir_Fecha_FormatoyyyMMdd(dtpBase2) + "'  <> ' ' Then '" + Funciones_Duke4.Funciones.Fun_Convertir_Fecha_FormatoyyyMMdd(dtpBase2) + "' Else fecha End  "
        ...
        protected void Fun_Calcular_Totales()
        {
            ...
            int c = 0;
                ...
                if (row.Visible)
                {
                    total = Funciones_Duke4.Funciones.Fun_Convierte_String_aDecimal(Convert.ToString(row.Cells["dgvmonto"].Value)) + total;
                    c++;
                }
        ...
        protected override void Presentar()
        {
            if (gridBase1.Rows.Count > 0)
            {
                ...
                {
                    cxcprecobroadelantado Presenta = new cxcprecobroadelantado();
                    Funciones_Duke4.Funciones.Fun_Habilitar_Control_Formulario(Presenta, false);

                    Presenta.Txtcobro.Text = gridBase1.SelectedRows[0].Cells["dgvsecuencia"].Value.ToString();
                    Presenta._secuencia = gridBase1.SelectedRows[0].Cells["dgvidcobro"].Value.ToString();
        ...
        protected override void Cancelar()
        {
            if (gridBase1.SelectedRows.Count > 0)
            {
                ...
                cancel.tabla = "cxcdbfcobroadelantado";
                ...
                cancel.datos();
                string cmd = string.Format("SELECT * FROM cxcdbfcobroadelantado where idcobro = '" + gridBase1.SelectedRows[0].Cells["dgvidcobro"].Value.ToString() + "'");
                DataSet ds = FuncionesSQL.Fun_Sql_Ejecutar(cmd);
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    {
                        if (Funciones_Duke4.Funciones.Fun_Convierte_String_aDecimal(ds.Tables[0].Rows[0]["balance"].ToString()) != Funciones_Duke4.Funciones.Fun_Convierte_String_aDecimal(ds.Tables[0].Rows[0]["monto"].ToString()))
                        {
