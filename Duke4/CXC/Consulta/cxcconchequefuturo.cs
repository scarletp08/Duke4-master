...
{
    public partial class cxcconchequefuturo : Duke4.MainFormConsulta
    {
        public cxcconchequefuturo()
        {
            InitializeComponent();
            string cmd = string.Format(queryconsulta + " cxcdbfchequefuturo.nulo=0 and cast(fecha as date) = cast(getdate() as date) order by secuencia");
            DataSet ds = FuncionesSQL.Fun_Sql_Ejecutar(cmd);
            gridBase1.DataSource = ds.Tables[0];
            Fun_Calcular_Totales();
        }
        
        string queryconsulta = "SELECT     secuencia, fecha,nombre,descripcion,nocheque,fechadeposito,total,idchequefuturo, dbo.cxcdbfchequefuturo.nulo, '' as entrada "
        + "FROM dbo.cxcdbfchequefuturo "
        + "inner JOIN dbo.cxcdbfcliente on cxcdbfcliente.idcliente= cxcdbfchequefuturo.idcliente "
        + "inner JOIN dbo.bandbfbanco on bandbfbanco.idbanco= cxcdbfchequefuturo.idbanco "
        + "where  ";
        ...
        protected override void Filtrar()
        {
            base.Filtrar();
            txtMayuscula1.Text = "";
            string cmd = string.Format(queryconsulta + " cxcdbfchequefuturo.fecha >= Case When '" + Funciones_Duke4.Funciones.Fun_Convertir_Fecha_FormatoyyyMMdd(dtpBase1) + "' <> ' ' Then '" + Funciones_Duke4.Funciones.Fun_Convertir_Fecha_FormatoyyyMMdd(dtpBase1) + "' Else cxcdbfchequefuturo.fecha End  "
            + " and cxcdbfchequefuturo.fecha <= Case When '" + Funciones_Duke4.Funciones.Fun_Convertir_Fecha_FormatoyyyMMdd(dtpBase2) + "'  <> ' ' Then '" + Funciones_Duke4.Funciones.Fun_Convertir_Fecha_FormatoyyyMMdd(dtpBase2) + "' Else cxcdbfchequefuturo.fecha End  "
        ...
        private void cxcconregistro_Load(object sender, EventArgs e)
        {
            ...
            Fun_Set_Nombre_Formulario("Consulta de Cheque de Factura", this);
        ...
        protected override void Presentar()
        {
            if (gridBase1.Rows.Count > 0)
            {
                ...
                {
                    cxcprechequefuturo Presenta = new cxcprechequefuturo();
                    Funciones_Duke4.Funciones.Fun_Habilitar_Control_Formulario(Presenta, false);

                    Presenta.Txtchequefuturo.Text = gridBase1.SelectedRows[0].Cells["dgvsecuencia"].Value.ToString();
                    Presenta._secuencia = gridBase1.SelectedRows[0].Cells["dgvidchequefuturo"].Value.ToString();
                    Presenta._nulo = Convert.ToBoolean(gridBase1.SelectedRows[0].Cells["dgvnulo"].Value.ToString());
                    Presenta.ShowDialog();
                }
                else
        ...
        protected override void Cancelar()
        {
            if (gridBase1.SelectedRows.Count > 0)
            {
                ...
                cancel.tabla = "cxcdbfchequefuturo";
                cancel.campo = "idchequefuturo";
                cancel.id = gridBase1.SelectedRows[0].Cells["dgvidchequefuturo"].Value.ToString();
                cancel.nombre = gridBase1.SelectedRows[0].Cells["dgvnombre"].Value.ToString();
                cancel.datos();
                string cmd = string.Format("SELECT * FROM cxcdbfchequefuturo where idchequefuturo = '" + gridBase1.SelectedRows[0].Cells["dgvidchequefuturo"].Value.ToString() + "' and nulo >0 ");
                DataSet ds = FuncionesSQL.Fun_Sql_Ejecutar(cmd);
