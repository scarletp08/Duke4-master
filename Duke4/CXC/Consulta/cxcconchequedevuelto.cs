...
{
    public partial class cxcconchequedevuelto : Duke4.MainFormConsulta
    {
        ...
        string queryconsulta = "SELECT     secuencia, fecha,cxcdbfregistro.referencia,nombre,totalcargo,total,idregistro, dbo.cxcdbfregistro.nulo, '' as entrada "
        + "FROM dbo.cxcdbfregistro "
        + "inner JOIN dbo.cxcdbfcliente on cxcdbfcliente.idcliente= cxcdbfregistro.idcliente "
        + "where cxcdbfregistro.tiporegistro=2 ";
        ...
        protected void Fun_Calcular_Totales()
        {
            ...
            int c = 0;
                ...
                if (row.Visible)
                {
                    cargo = Funciones_Duke4.Funciones.Fun_Convierte_String_aDecimal(Convert.ToString(row.Cells["dgvcargo"].Value)) + cargo;
                    total = Funciones_Duke4.Funciones.Fun_Convierte_String_aDecimal(Convert.ToString(row.Cells["dgvtotal"].Value)) + total;
                    c++;
                }

            }
            Txtcargo.Text = Convert.ToString(cargo);
        ...
        protected override void Presentar()
        {
            if (gridBase1.Rows.Count > 0)
            {
                ...
                {
                    cxcprechequedevuelto Presenta = new cxcprechequedevuelto();
                    Funciones_Duke4.Funciones.Fun_Habilitar_Control_Formulario(Presenta, false);

                    Presenta.Txtchequedevuelto.Text = gridBase1.SelectedRows[0].Cells["dgvsecuencia"].Value.ToString();
                    Presenta._secuencia = gridBase1.SelectedRows[0].Cells["dgvidregistro"].Value.ToString();
