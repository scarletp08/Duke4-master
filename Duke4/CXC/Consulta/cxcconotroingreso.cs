...
{
    public partial class cxcconotroingreso : Duke4.MainFormConsulta
    {
        public cxcconotroingreso()
        {
            InitializeComponent();
            string cmd = string.Format(queryconsulta + " nulo=0 and cast(fecha as date) = cast(getdate() as date) order by secuencia");
            DataSet ds = FuncionesSQL.Fun_Sql_Ejecutar(cmd);
            gridBase1.DataSource = ds.Tables[0];
            Fun_Calcular_Totales();
        }
        
        string queryconsulta = "SELECT     secuencia, fecha,referencia,beneficiario,monto,idotroingreso,nulo,'' as entrada "
        + "FROM dbo.cxcdbfotroingreso "
        + "where ";
        ...
        protected override void Filtrar()
        {
            ...
            + " and beneficiario = Case When '" + Txtnombrecliente.Text + "' <> '' Then '" + Txtnombrecliente.Text + "'   Else beneficiario End "
        ...
        protected override void Presentar()
        {
            if (gridBase1.Rows.Count > 0)
            {
                ...
                {
                    cxcpreotroingreso Presenta = new cxcpreotroingreso();
                    Funciones_Duke4.Funciones.Fun_Habilitar_Control_Formulario(Presenta, false);

                    Presenta.Txtotroingreso.Text = gridBase1.SelectedRows[0].Cells["dgvsecuencia"].Value.ToString();
                    Presenta._secuencia = gridBase1.SelectedRows[0].Cells["dgvidotroingreso"].Value.ToString();
        ...
        protected void GeneraReporte(string opcion, int pos, bool nulo)
        {
            string ruta = "";
            string nombredoc = "";
            Reporte_General.CXC.cxcrepotroingreso01 Reporte = new Reporte_General.CXC.cxcrepotroingreso01();
            ...
            Reporte._id = gridBase1.Rows[pos].Cells["dgvidotroingreso"].Value.ToString();
            Reporte.Fun_Reporte();
