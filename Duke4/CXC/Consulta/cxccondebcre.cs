...
{
    public partial class cxccondebcre : Duke4.MainFormConsulta
    {
        public cxccondebcre()
        {
            InitializeComponent();
            string cmd = string.Format(queryconsulta + " cxcdbfdebcre.nulo=0 and cast(cxcdbfdebcre.fecha as date) = cast(getdate() as date) order by secuencia");
            DataSet ds = FuncionesSQL.Fun_Sql_Ejecutar(cmd);
            gridBase1.DataSource = ds.Tables[0];
            Fun_Calcular_Totales();
        }
        
        string queryconsulta = "SELECT     cxcdbfdebcre.secuencia, cxcdbfdebcre.fecha,cxcdbfregistro.secuencia as secuenciafactura,nombre,cxcdbfdebcre.ncf,cxcdbfdebcredetalle.ncffactura,cxcdbfdebcre.gravado,cxcdbfdebcre.itbis,cxcdbfdebcre.total,cxcdbfdebcre.idregistro, dbo.cxcdbfdebcre.nulo, tiponota, '' as entrada "
        + "FROM dbo.cxcdbfdebcre "
        + "inner join dbo.cxcdbfdebcredetalle on cxcdbfdebcredetalle.idregistro= cxcdbfdebcre.idregistro "
        + "inner JOIN dbo.cxcdbfcliente on cxcdbfcliente.idcliente= cxcdbfdebcre.idcliente "
        + "inner join dbo.cxcdbfregistro on cxcdbfregistro.idregistro= cxcdbfdebcredetalle.idfactura "
        + "where ";
        ...
        protected override void Filtrar()
        {
            ...
            txtMayuscula1.Text = "";
            string cmd = string.Format(queryconsulta + " cxcdbfdebcre.fecha >= Case When '" + Funciones_Duke4.Funciones.Fun_Convertir_Fecha_FormatoyyyMMdd(dtpBase1) + "' <> ' ' Then '" + Funciones_Duke4.Funciones.Fun_Convertir_Fecha_FormatoyyyMMdd(dtpBase1) + "' Else cxcdbfdebcre.fecha End  "
            + " and cxcdbfdebcre.fecha <= Case When '" + Funciones_Duke4.Funciones.Fun_Convertir_Fecha_FormatoyyyMMdd(dtpBase2) + "'  <> ' ' Then '" + Funciones_Duke4.Funciones.Fun_Convertir_Fecha_FormatoyyyMMdd(dtpBase2) + "' Else cxcdbfdebcre.fecha End  "
            ...
            + " and cxcdbfdebcre.idmoneda ='" + cmbBase1.SelectedValue + "' and tiponota='"+tiponota+"' "
            + " order by secuencia");
        ...
        {
            double itbis = 0, total = 0,gravado=0;
            int c = 0;
                ...
                if (row.Visible)
                {
                    gravado = Funciones_Duke4.Funciones.Fun_Convierte_String_aDecimal(Convert.ToString(row.Cells["dgvgravado"].Value)) + gravado;
            ...
            Txtgravado.Text= Convert.ToString(gravado);
        ...
        protected override void Presentar()
        {
            if (gridBase1.Rows.Count > 0)
            {
                ...
                {
                    cxcpredebcre Formulario = new cxcpredebcre();
                    Funciones_Duke4.Funciones.Fun_Habilitar_Control_Formulario(Formulario, false);

                    Formulario.Txtregistro.Text = gridBase1.SelectedRows[0].Cells["dgvsecuencia"].Value.ToString();
                    Formulario._secuencia = gridBase1.SelectedRows[0].Cells["dgvidregistro"].Value.ToString();
                    Formulario._nulo = Convert.ToBoolean(gridBase1.SelectedRows[0].Cells["dgvnulo"].Value.ToString());
                    Formulario.Rbncredito.Checked = Rbncredito.Checked;
                    Formulario.Rbndebito.Checked = Rbndebito.Checked;
